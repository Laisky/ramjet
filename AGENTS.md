...
Ramjet aims to provide a Python‑based scheduling framework. All subtasks are declared in `ramjet/tasks/__init__.py`. Every task can use the utility functions `bind_task` and `bind_handle`: `bind_task` registers the task for periodic execution, and `bind_handle` registers a web handler, enabling the task to expose an HTTP API.
